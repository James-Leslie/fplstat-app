...
| finished | bool | |
| is_current | bool | |
| average_entry_score | int4 | nullable (future GWs) |
| highest_score | int4 | nullable (future GWs) |
...
| bonus | int4 | |
| bps | int4 | |
| influence | numeric | cast from String |
| creativity | numeric | cast from String |
| threat | numeric | cast from String |
| ict_index | numeric | cast from String |
| expected_goals | numeric | cast from String |
| expected_assists | numeric | cast from String |
| expected_goal_involvements | numeric | cast from String |
| expected_goals_conceded | numeric | cast from String |
| value | int4 | price at time of GW (× 10) |
| selected | int4 | ownership count |
| transfers_in | int4 | GW transfers in |
| transfers_out | int4 | GW transfers out |
| pts_rolling_3gw | int4 | **derived** — rolling sum last 3 GWs |
| pts_rolling_5gw | int4 | **derived** — rolling sum last 5 GWs |

### `fixtures`
| column | type | notes |
|---|---|---|
| id | int4 PK | FPL fixture id |
| gameweek_id | int4 FK → gameweeks.id | nullable (blank gameweeks) |
